...
**Q：为什么上传失败提示 “Image exists”？**  
A：sm.ms 不允许重复内容上传。请确保文件内容唯一。

**Q：能否上传非图片文件？**  
A：sm.ms 仅支持图片（jpg/png/gif/webp 等），其他类型会被拒绝。

**Q：删除文件后，sm.ms 上的图片还能访问吗？**  
A：可以！sm.ms 删除接口**仅移除你的管理权限**，原图仍可公开访问（除非被系统清理）。
